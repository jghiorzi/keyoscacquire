# -*- coding: utf-8 -*-
"""
Script to test obtaining data with different waveform format for Keysight DSO2024A
...
for wav_format in wformat:
    ...
    time, vals = koa.dataprocessing.process_data(
        scope._raw, scope._metadata, wav_format, verbose_acquistion=True
    )
    times[0].append(time)
...
inst.write("*CLS")  # clears the device
...
for wav_format in wformat:
    ...
    inst.write(":WAVeform:FORMat " + wav_format)
    # MSBF is default, must be overridden for WORD to work
    inst.write(":WAVeform:BYTeorder LSBFirst")
    # Make sure the scope is sending signed ints
    inst.write(":WAVeform:UNSigned OFF")
    ...
    if wav_format in ["WORD", "BYTE"]:
        ...
        raw_bin = inst.query_binary_values(
